...
# Note: twist_meanpool_e100 was an attempted twist, but it is excluded from the
# final report figures because AutoAugment became the main Modified ViT result.
...
TWIST_EXPERIMENTS = ["vit_baseline_e100", "twist_autoaugment_e100"]
...
- `model_test_accuracy.png`: test accuracy of all final report experiments.
- `cnn_vs_vit_accuracy.png`: CNN baseline vs ViT baseline vs AutoAugment ViT.
- `training_time_comparison.png`: training time comparison for all final report experiments.
...
- `twist_accuracy.png`: baseline ViT vs AutoAugment ViT comparison.
- `twist_val_curve.png`: validation curve comparison for the final twist only.

## Excluded from final report figures

- `twist_meanpool_e100`: attempted twist kept in raw outputs, but not used as the final Modified ViT result.
